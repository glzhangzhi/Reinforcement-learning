...
class Net(nn.Module):
    def __init__(self, n_states, n_actions, n_hidden):
        ...
        self.fc1 = nn.Linear(n_states, n_hidden)  # y = w * x + b
...
class DQN(object):
    def __init__(self, n_states, n_actions, n_hidden, batch_size, lr, epsilon,
        ...
        self.eval_net, self.target_net = Net(n_states, n_actions, n_hidden), \
            Net(n_states, n_actions, n_hidden)
    ...
    def learn(self):
        ...
        b_state = torch.tensor(b_memory[:, self.n_states], dtype=torch.float32)
        ...
        b_reward = torch.Tensor(
            ...
            dtype=torch.float)
        b_next_state = torch.Tensor(b_memory[:, -self.n_states:],
                                    dtype=torch.float)
...
for i_episode in range(n_episodes):
    ...
    while True:
        ...
        r2 = (env.theta_threshold_radians - abs(theta)) / \
            env.theta_threshold_radians - 0.5  # 柱子越正越好
        reward = r1 + r2
        ...
        if done:
            print('Episode finished after {} timesteps,'
                  'total rewards {}'.format(t+1, rewards))
            break
